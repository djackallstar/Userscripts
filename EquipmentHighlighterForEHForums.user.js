...
if(typeof p_set == 'undefined')
{
    var p_set = // Add [[pattern_1, pattern_2, ... , pattern_n], color] to the array by yourself.
    [
        [
            [   // Rapier
...
for(var i=posts.length-1; i>=0; i--) {
    ...
    for(var j=lnks.length-1; j>=0; j--) {
        for (var k=0, len=p_set.length; k<len; k++) {
            var highlighted = false
            for(var m=p_set[k][0].length-1; m>=0; m--) {
                if(p_set[k][0][m].test(lnks[j].text)) {
                    highlighted = true
                    if(lnks[j].querySelector('span')) { lnks[j].innerHTML = lnks[j].querySelector('span').innerHTML }
